        ...
        content: Row(
          children: [
            if (tone != AmdsStatusTone.neutral) ...[
              Icon(tone.icon, size: AmdsSize.iconSm, color: Theme.of(context).colorScheme.onInverseSurface),
              const SizedBox(width: AmdsSpacing.sm),
            ],
            Expanded(child: Text(message)),
          ],
        ),
        duration: duration ?? (tone == AmdsStatusTone.danger || onAction != null ? const Duration(seconds: 6) : const Duration(seconds: 4)),
  ...
  static Future<bool> confirm(
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        icon: danger ? Icon(Icons.warning_amber_rounded, color: context.amds.colors.danger) : null,
        title: Text(title),
        content: Text(body),
        actions: [
          AmdsButton(
            ...
            autofocus: danger, // destructive dialogs default-focus the safe choice
            onPressed: () => Navigator.of(context).pop(false),
          ),
          AmdsButton(
            label: confirmLabel,
            variant: danger ? AmdsButtonVariant.destructive : AmdsButtonVariant.primary,
            autofocus: !danger,
